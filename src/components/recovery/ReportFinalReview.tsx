    ...
    <div className="panel-corners flex flex-1 flex-col rounded-[6px] border border-border bg-card p-5 shadow-[var(--shadow-sm)]">
      <div className="mb-4 flex shrink-0 items-start justify-between gap-3">
        <div>
          ...
          <h3 className="mt-0.5 text-[15px] font-extrabold text-ink">Damage Assessment Report — {clusterId}</h3>
        </div>
        <button
          type="button"
          onClick={onBack}
          ...
          {sections.map((section) => {
            ...
            return (
              <div key={section.id} className="py-3.5 first:pt-0 last:pb-0">
                <h4 className="text-[12.5px] font-extrabold text-ink">{section.title}</h4>
                <p className="mt-1.5 text-[13px] leading-relaxed text-ink">{state?.content}</p>
                {state?.timeInfo && (
