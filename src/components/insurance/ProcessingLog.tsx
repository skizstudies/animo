...
export function ProcessingLog({ log }: ProcessingLogProps) {
  return (
    <div className="rounded-[6px] border border-divider bg-bg px-4 py-3">
      <span className="font-mono text-[10px] font-bold tracking-[1px] text-ink-soft uppercase">Processing log</span>
      <ul className="mt-2 flex flex-col gap-2">
        {STAGE_ORDER.map((stage) => {
          const entry = log.find((l) => l.stage === stage);
          return (
            <li key={stage} className="flex items-center gap-2.5 text-[12px]">
              <span
